...
- [x] **7.1** Убрать фильтр длины (`char_length`) — для HSK он неактуален
  - Либо оставить, но переименовать в «Кол-во иероглифов» и убрать из поиска (оставить только как опциональный фильтр)
- [x] **7.2** Поиск: `WHERE chinese LIKE %q% OR russian_word LIKE %q%` — оставить как есть, работает для HSK
- [x] **7.3** `GET /dictionary/random/any` — если не нужно для HSK, можно удалить роут
- [x] **7.4** `GET /dictionary/search` — по умолчанию искать только `source='hsk'` (или source не фильтровать — в БД только HSK)
...
#### 14. ⚙️ .env
- [x] **14.1** Скопировать `.env.example` → `.env`
- [x] **14.2** Проверить, что `dotenv` загружается в `app.js` (уже есть)
- [x] **14.3** Добавить `.env` в `.gitignore`

#### 15. 🚦 Rate limiting
- [x] **15.1** Установить `express-rate-limit`
- [x] **15.2** Добавить глобальный лимит: `max: 100`, `windowMs: 15 * 60 * 1000`
- [x] **15.3** Для `/api/dictionary/search` — лимит 30 запросов/мин (тяжёлый поиск)
- [x] **15.4** Для `/api/study-lists/:id/review` — лимит 60 запросов/мин

#### 16. 🧪 Тесты
- [x] **16.1** Создать `tests/api.test.js` с использованием `supertest`
- [x] **16.2** Тест: health endpoint возвращает 200
- [x] **16.3** Тест: создание списка + добавление слова + получение слов
- [x] **16.4** Тест: импорт HSK уровня
- [x] **16.5** Тест: SRS review submission
- [x] **16.6** Тест: поиск по словарю
- [x] **16.7** Тест: валидация (400 на неверные данные)
- [x] **16.8** Добавить скрипт `npm test` в `package.json`