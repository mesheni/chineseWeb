...
### 12. ✅ Review mode: кнопки качества (1-5) без качества 2 — ИСПРАВЛЕНО
...
**Статус:** ✅ Выполнено. В `public/index.html` (строка 126) добавлена кнопка `<button id="reviewQuality2" class="review-quality hidden" data-quality="2">Почти</button>`. В `public/js/app.js` (строка 32) `reviewQuality2` добавлен в список элементов. В `public/css/style.css` (строки 596-597) добавлены стили: `#reviewQuality2 { background: #f97316; }` и hover `#ea580c` — оранжевый, между красным (quality1) и жёлтым (quality3). Так как используется `document.querySelectorAll('.review-quality')`, новая кнопка автоматически обрабатывается существующим обработчиком.
**Шаги реализации:**
...
### 13. ✅ Study mode: пиньинь всегда виден на лицевой стороне карточки — ИСПРАВЛЕНО
...
**Статус:** ✅ Выполнено. В `showStudyWord()` (строка 388) пиньинь обнуляется: `e.studyPinyin.textContent = '';`. В обработчике `studyShowAnswer` (строка 404) пиньинь устанавливается: `e.studyPinyin.textContent = word.pinyin || '';` — после флипа карточки.
**Шаги реализации:**
...
### 14. ✅ HSK import: нет индикатора прогресса — ИСПРАВЛЕНО
...
**Статус:** ✅ Выполнено.
- **Сервер:** В `server/routes/studyLists.js` (строки 253-262) цикл `for` заменён на `bulkCreate` батчами по 500 записей с `ignoreDuplicates: true`. Уникальный индекс `list_id, dictionary_id` в модели `StudyListWord` обеспечивает корректную обработку дубликатов.
- **Фронт:** В `public/js/app.js` обработчик импорта показывает анимацию прогресса через `setInterval` (меняет текст кнопки `⏳ Импорт·`, `··`, `···`), а после завершения показывает `X из Y слов` в alert.
**Шаги реализации:**
...
### 15. ✅ Dictionary: `definition` всегда пустой для HSK-данных — ИСПРАВЛЕНО
...
**Статус:** ✅ Выполнено. В `renderDictResults()` (`public/js/app.js`) блок `.dict-definition` теперь рендерится условно: `${sample.definition ? `<div class="dict-definition">${escHtml(sample.definition).slice(0, 200)}</div>` : ''}`. Записи без definition не показывают пустой блок.
**Шаги реализации:**
...
### 16. ✅ Test mode: опции-дистракторы слишком простые — ИСПРАВЛЕНО
...
**Статус:** ✅ Выполнено. В `generateTestOptions()` (`public/js/app.js`) реализован вариант А — дистракторы сортируются по близости длины перевода к правильному ответу. Алгоритм: каждому wrong-варианту присваивается `diff = |text.length - correct.length|`, сортировка по `diff` (с дополнительной случайной составляющей для разнообразия), берутся 3 ближайших, затем итоговый порядок перемешивается `shuffleArray`. Дистракторы теперь похожи по длине на правильный ответ.
**Шаги реализации:**
...
### 17. ✅ CSS: нет активного состояния для кнопок навигации на мобильных — ИСПРАВЛЕНО
...
**Статус:** ✅ Выполнено. В `public/css/style.css` после правила `.nav-tab.active .nav-icon` добавлено правило `.nav-tab:active { background: var(--bg-card); }`. На мобильных устройствах при нажатии на таб появляется серый фон.
**Шаги реализации:**
...
### 18. ✅ Дизайн: нет переключателя тёмная/светлая тема — ИСПРАВЛЕНО
**Описание:** Всегда тёмная тема. Нет accessibility для пользователей, предпочитающих светлый интерфейс.
**Статус:** ✅ Выполнено. В `public/css/style.css` добавлен блок `:root.light` со всеми CSS-переменными для светлой темы. В `public/index.html` в header добавлена кнопка `#themeToggle`. В `public/js/app.js` реализован обработчик: переключение класса `light` на `<html>`, сохранение выбора в `localStorage`, иконка 🌙/☀️. Функция `applyTheme()` вызывается при загрузке.
**Шаги реализации:**
...
### 19. ✅ Дизайн: нет клавиатурных сокращений — ИСПРАВЛЕНО
**Описание:** Нельзя использовать Enter для флипа карточки, цифры 1-5 для качества в review, стрелки для навигации.
**Статус:** ✅ Выполнено. В `public/js/app.js` добавлен глобальный обработчик `keydown`. Реализованы сокращения: Enter в словаре (поиск), Space/Enter в study (флип/следующее), Space/Enter в review (показать ответ), 1-5 в review (качество), 1-4 в test (выбор варианта). Обработчик игнорирует нажатия при фокусе в `input`/`select`/`textarea`.
**Шаги реализации:**
...
### 20. ✅ Дизайн: иконки навигации — emoji, а не SVG — ИСПРАВЛЕНО
**Описание:** Emoji выглядят по-разному на разных ОС.
**Статус:** ✅ Выполнено. В `public/index.html` все 6 emoji в навигации заменены на inline SVG: лупа (Словарь), список (Списки), книга (Учить), стрелки (Повтор), мишень (Тест), гистограмма (Стата). SVG используют `stroke="currentColor"`, что позволяет наследовать цвет активной вкладки.
**Шаги реализации:**
...
### 21. ✅ Дизайн: нет favicon — ИСПРАВЛЕНО
**Описание:** Вкладка браузера показывает пустую иконку.
**Статус:** ✅ Выполнено. Создан `public/favicon.svg` с иероглифом 汉 на тёмном фоне (#0f172a) и голубым цветом (#38bdf8). В `public/index.html` в `<head>` добавлен `<link rel="icon" type="image/svg+xml" href="/favicon.svg">`. Статические файлы раздаются через `express.static('public')` в `server/app.js`.
**Шаги реализации:**
...
### 22. ✅ Дизайн: карточка в тестовом режиме использует тот же `#test-card`, но без флипа — ИСПРАВЛЕНО
**Описание:** В test mode карточка показывает иероглиф + пиньинь, но это не flashcard.
**Статус:** ✅ Выполнено. Проверено: в `public/index.html` `#test-card` уже не имеет задней стороны (`.card-back`) и не использует анимацию флипа — это простой блок с иероглифом и пиньинем. В CSS отсутствуют специфические стили для `#test-card` (perspective, transform-style). Упрощение не требуется — текущая реализация корректна.
**Шаги реализации:**
...
### 23. ✅ UX: нет подтверждения при выходе из review/study/test — ИСПРАВЛЕНО
**Описание:** При переключении вкладки во время сессии прогресс теряется.
**Статус:** ✅ Выполнено. В `public/js/app.js` в обработчике переключения вкладок добавлена проверка: если активна сессия study/review/test (контент не скрыт) и пользователь переключается на другую вкладку, показывается `confirm('Прогресс текущей сессии будет потерян. Продолжить?')`. При отмене переключение не происходит.
**Шаги реализации:**
...
### 24. ✅ UX: статистика слишком базовая — ИСПРАВЛЕНО
**Описание:** Только 3 числа: всего слов, изучено, на сегодня. Нет графиков.
**Статус:** ✅ Выполнено. Добавлен streak в `GET /:id/stats` (вычисляется на лету из `last_review`). Добавлен эндпоинт `GET /:id/stats/daily` для данных за 30 дней. Подключён Chart.js CDN (`chart.js@4.4.7`) в `index.html`. На фронте рендерятся: линейный график активности за 30 дней и круговая диаграмма (pie chart) распределения слов (новые/изученные/проблемные). Добавлено 4-е число в карточках: streak.
**Шаги реализации:**
...
### 25. ✅ UX: нет сортировки слов в просмотре списка — ИСПРАВЛЕНО
**Описание:** `GET /:id/words` возвращает слова в порядке `id DESC`. Нет сортировки.
**Статус:** ✅ Выполнено. В `GET /:id/words` добавлена поддержка query-параметров `sort` (chinese, created, next_review) и `order` (asc/desc). На фронте в `listDetail` добавлен `<select>` для выбора сортировки. При изменении сортировки список перезагружается.
**Шаги реализации:**
...
### 26. ✅ UX: словарь не показывает уровень HSK — ИСПРАВЛЕНО
**Описание:** В результатах поиска нет информации об уровне HSK.
**Статус:** ✅ Выполнено. В `renderDictResults()` добавлен бейдж `<span class="dict-hsk-badge">HSK ${hsk_level}</span>` (отображается только если `hsk_level` не null). Стиль `.dict-hsk-badge` уже присутствовал в CSS.
**Шаги реализации:**
...
| 🟠 P4 (UX important) | #14 (прогресс импорта), #26 ✅ (HSK бейджи), #18 (тёмная/светлая тема), #25 ✅ (сортировка слов) | 3 часа |
